...
include "fftw3.f"
   ...
   allocate( fft_cmplx_in(nfft), fft_out(nfft), fft_out2(nfft) )
   !
   ...
   !
   forall (i=1:npts) fft_cmplx_in(i) = complex(data_in(2,i), data_in(3,i) ) * r_nfft
   fft_cmplx_in(npts+1:nfft) = complex(0.D0, 0.D0)
   !
   ! do fft to get cross-correlation
   call dfftw_execute_dft(pFW, fft_cmplx_in, fft_out)
   fft_out2 = fft_out
   !
   forall (i=1:npts) fft_cmplx_in(i) = complex(data_in(4,i), data_in(5,i) ) * r_nfft
   fft_cmplx_in(npts+1:nfft) = complex(0.D0, 0.D0)
   call dfftw_execute_dft(pFW, fft_cmplx_in, fft_out)
   !
   forall (i=1:nfft) fft_out(i) = conjg(fft_out2(i))*fft_out(i)
   call dfftw_execute_dft(pBW, fft_out, fft_cmplx_in)
   !
   do i = 1, nacf
      data_in(2,i) = real(fft_cmplx_in(i))/dble(npts-i+1)
      data_in(3,i) = imag(fft_cmplx_in(i))/dble(npts-i+1)
   enddo
   !
   ...
   fmtstr = '("# time-lag",10x,"ccf_of_col_",I2.2,"_&_",I2.2)'
   write(11,fmtstr) icol(2:3)
   fmtstr = '(G19.10,2(1x,G19.10))'
   data_in(1,1:nacf) = data_in(1,1:nacf) - data_in(1,1)
   do i = 1, nacf
      write(11, fmtstr) data_in(1:3,i)
   enddo
   close(11)
